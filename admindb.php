...
// Total Events
$result_events = $connection->query("SELECT COUNT(*) as total FROM Events");
...
// Total Participants
$result_participants = $connection->query("SELECT COUNT(*) as total FROM Participants");
$total_participants = $result_participants->fetch_assoc()['total'];
...
<header>
    ...
    <nav class="navbar">
        ...
        <a href="adminupcoming.php"><i class="fa fa-clock"></i></a>
    </nav>
...
<div class="main-content">
    ...
    <div class="card-container">
        <a href="events.php" class="card">
            <h3><i class="fa fa-calendar-days"></i> Total Events</h3>
            <p><?= $total_events ?></p>
        </a>
        <a href="organizers.php" class="card">
            <h3><i class="fa fa-user-tie"></i> Organizers</h3>
            <p><?= $total_organizers ?></p>
        </a>
        <a href="participants.php" class="card">
            <h3><i class="fa fa-users"></i> Participants</h3>
            <p><?= $total_participants ?></p>
        </a>
        <a href="adminupcoming.php" class="card">
            <h3><i class="fa fa-clock"></i> Upcoming</h3>
            <p><?= $upcoming_events ?></p>
        </a>
    </div>

    <div class="dashboard-bottom" style="display:flex; flex-wrap:wrap; gap:20px;">
        <!-- Calendar -->
...
<script>
    ...
    function renderCalendar() {
        ...
        for (let day = 1; day <= lastDate; day++) {
            ...
            calendar.innerHTML += `<div class="calendar-day ${hasEvent ? 'has-event' : ''}" onclick="showEvents('${dateStr}')">${day}</div>`;
        }
    }
    ...
    function showEvents(date) {
        ...
        events.forEach(e => list.innerHTML += `<li>📌 ${e.EventName}</li>`);
    }
